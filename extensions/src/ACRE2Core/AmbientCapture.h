...
public:
    ...
    /*
     * DEVELOPMENT ONLY -- compiled out unless ACRE2_AMBIENT_DEV_TOOLS is set.
     *
     * Writes ambience and microphone to separate channels of a stereo file,
     * both captured before they are summed, so a different DSP chain can be
     * tried offline against real gameplay rather than against the Phase 0
     * takes. The ambience channel is taken before the noise gate, so gate
     * settings can be varied offline too.
     *
     * This records the player's raw microphone to disk. That is fine on a
     * development machine and unacceptable anywhere else, which is why it is a
     * compile-time opt-in rather than a setting somebody could leave on.
     */
